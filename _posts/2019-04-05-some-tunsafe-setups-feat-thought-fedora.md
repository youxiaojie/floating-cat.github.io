...
# 获取这个 group 的 GID（注释里 yyyy 字段），该 group id 用于之后的路由配置中
...
-j MARK --set-mark 1
# 让这些打了标记的网络包走 TunSafe/凯铂莱 过
sudo ip rule add fwmark 1 table 100
sudo ip route add table 100 default dev $cavorite_interface
# 现在走到 TunSafe/凯铂莱 这个设备接口的源 IP 都是本地的 IP
# 我们需要将这些本地的 IP NAT 成这个 TunSafe/凯铂莱 这个设备接口的 IP
sudo iptables -t nat -A POSTROUTING -o $cavorite_interface -j MASQUERADE
# 现在从 TunSafe/凯铂莱 这个设备接口出去的网络包的 TCP 最大报文长度
...
因为 iptables 没有办法根据应用名、进程 ID 来进行路由选择（不知道为什么相应的功能在旧的 iptables 上移除了[16]），所以这也是为什么本文选择了白名单的策略的原因之一——没办法根据应用名来进行黑名单会导致配置黑名单变得超级麻烦……所以我们现在使用应用运行时所使用的 group id，来进行路由选择。当应用运行的 group 是 cavorite 的时候，该应用可以通过 TunSafe/凯铂莱 来进行网络请求。结合下面的这个脚本，这意外的是一个很不错的解决方案。
...
现在，我们可以通过这个脚本来随意地将要运行的应用加入到白名单下。比如说我们可以使用 `t wget https://example.com/` 来通过 TunSafe/凯铂莱 访问 example 这个网站；通过 GUI 或者文本编辑器来修改应用程序的 .desktop 文件，来将想要运行的桌面应用添加到白名单内:
...
t bash
# 进入白名单模式
# 通过 TunSafe/凯铂莱 访问 example 这个网站
...
我们的 DNS 策略也比较简单。让所有目的地端口是 53 （DNS 解析使用 53 端口）并且 group 是 cavorite 的网络请求走 TunSafe/凯铂莱 过：
...
cavorite_interface=tun0
cavorite_gid=yyyy
# 这里的 cavorite_dns 可以是我们用的 TunSafe/凯铂莱 服务器所使用的 DNS 解析服务商的 IP
# 或者其他和我们服务器的网络向性比较好的 DNS 解析服务商的 IP
# 也可以将我们的 TunSafe/凯铂莱 的服务器配置成 DNS 服务器
...
-m owner --gid-owner $cavorite_gid -j DNAT --to-destination $cavorite_dns
# 让该新的 DNS 解析服务商的 IP 的网络请求走 TunSafe/凯铂莱 过
